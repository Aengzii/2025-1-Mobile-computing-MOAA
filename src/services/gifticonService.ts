...
export const saveGifticon = async (
  ...
  console.log('saveGifticon called with originalImageUri:', originalImageUri);
  ...
  try {
    ...
    if (originalImageUri.startsWith('content://')) {
      // Content URI는 확장자가 없으므로 기본값 사용
      fileExtension = 'jpg';
    } else if (originalImageUri.includes('.')) {
      // 일반 파일 경로에서만 확장자 추출
      const parts = originalImageUri.split('.');
      const lastPart = parts[parts.length - 1];
      if (lastPart && lastPart.length <= 4) { // 확장자는 보통 3-4자
        fileExtension = lastPart.split('?')[0] || 'jpg';
      }
    }
    ...
    // Content URI 처리 방식 개선
    if (originalImageUri.startsWith('content://')) {
      // Content URI는 copyFile 사용
      try {
        await RNFS.copyFile(originalImageUri, newImagePath);
        console.log('Successfully copied content URI to:', newImagePath);
      } catch (copyError) {
        console.error('copyFile failed, trying readFile:', copyError);
        // copyFile 실패시 readFile 시도
        const fileContent = await RNFS.readFile(originalImageUri, 'base64');
        await RNFS.writeFile(newImagePath, fileContent, 'base64');
      }
    } else {
      // 일반 파일은 기존 방식 유지
      const fileContent = await RNFS.readFile(originalImageUri, 'base64');
    ...
    if (newImagePath) {
      try {
        const exists = await RNFS.exists(newImagePath);
        if (exists) {
          await RNFS.unlink(newImagePath);
          console.log('Rolled back failed save');
        }
      } catch (err) {
        console.error('오류 롤백 파일 삭제 실패:', err);
      }
    }
