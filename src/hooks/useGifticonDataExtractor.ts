...
const useGifticonDataExtractor = (): UseGifticonDataExtractorReturn => {
  ...
  const processImageForGifticonData = async (imageUri: string): Promise<ExtractedGifticonInfo | null> => {
    ...
    if (!imageUri.startsWith('file://') && !imageUri.startsWith('content://')) {
      processedUri = `content://${imageUri}`;
    } else if (imageUri.startsWith('file://')) {
      // 기존 file:// 처리 로직 유지 (필요하다면)
      processedUri = imageUri; // 또는 file:// 스키마를 제거하는 로직
    } else {
      processedUri = imageUri; // content:// 스키마는 그대로 사용
    }
    ...
    try {
      ...
      if (e instanceof Error) {
        if (e.message.includes('text recognition')) {
          errorMessage = "이미지에서 텍스트를 인식하는 데 실패했습니다.\n이미지 품질을 확인해주세요.";
        } else if (e.message.includes('barcode scanning')) {
          errorMessage = "이미지에서 바코드를 스캔하는 데 실패했습니다.\n바코드 영역이 선명한지 확인해주세요.";
        } else {
