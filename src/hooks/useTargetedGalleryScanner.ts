...
const useTargetedGalleryScanner = (): UseTargetedGalleryScannerReturn => {
  ...
  const startScan = useCallback(async (options: GalleryScanOptions) => {
    ...
    // !!! 디버깅용: lastScannedTimestamp 강제 초기화 !!!
    const storageKeySuffixForced = options.targetAlbum ? `album_${options.targetAlbum.replace(/\s+/g, '_')}` : 'all_gallery';
    const lastScannedTimestampKeyForced = `${LAST_SCANNED_TIMESTAMP_KEY_PREFIX}${storageKeySuffixForced}`;
    try {
        console.warn(`[DEBUG] 강제 초기화 시도: AsyncStorage key ${lastScannedTimestampKeyForced} 값을 '0'으로 설정합니다.`);
        await AsyncStorage.setItem(lastScannedTimestampKeyForced, '0');
        console.warn('[DEBUG] 강제 초기화 완료.');
    } catch (e) {
        console.error('[DEBUG] 강제 초기화 실패:', e);
    }
    // !!! 디버깅용 코드 끝 !!!
    ...
    if (scanMode === 'newInAlbum' || scanMode === 'newInGallery') {
      ...
      if (lastScannedTimestamp > currentTime) {
        console.warn(`[TargetedGalleryScanner] Detected future last scanned timestamp: ${lastScannedTimestamp}. Resetting to 0.`);
        lastScannedTimestamp = 0; // Reset if the timestamp is in the future
        try {
            // Also remove the invalid entry from AsyncStorage to prevent future issues
 await AsyncStorage.setItem(lastScannedTimestampKey, '0');
 console.log(`[TargetedGalleryScanner] Resetting lastScannedTimestamp for key ${lastScannedTimestampKey} in AsyncStorage to 0.`);
        } catch (e) {
            console.error(`[TargetedGalleryScanner] Failed to reset lastScannedTimestamp in AsyncStorage for key ${lastScannedTimestampKey}:`, e);
        }
      }
    }
    ...
    try {
      ...
      console.log('CameraRoll.getPhotos params:', params);
        ...
        for (const edge of photosResult.edges) {
          ...
          if (extractedInfo) {
            console.log('OCR Result:', {
              barcodeValue: extractedInfo.barcodeValue,
              brandName: extractedInfo.brandName,
              productName: extractedInfo.productName,
              expiryDate: extractedInfo.expiryDate,
              fullTextLength: extractedInfo.fullText?.length || 0
            });
            
            // 바코드가 있는 경우에만 저장
            if (extractedInfo.barcodeValue) {
              const isDuplicate = await checkDuplicateBarcode(extractedInfo.barcodeValue);
              
              if (!isDuplicate) {
                console.log('Saving gifticon with barcode:', extractedInfo.barcodeValue);
                
                // saveGifticon에 전달할 데이터 준비
                const dataToSave = {
                  barcodeValue: extractedInfo.barcodeValue,
                  brandName: extractedInfo.brandName || '정보없음',
                  productName: extractedInfo.productName || '정보없음',
                  expiryDate: extractedInfo.expiryDate || null,
                  amount: extractedInfo.amount || null,
                  fullText: extractedInfo.fullText
                };
                
                // 이제 saveGifticon 호출
                const savedGifticon = await saveGifticon(dataToSave, imageUri);
                
                if (savedGifticon && isMountedRef.current) {
                  setScannedAndSavedGifticons(prev => [...prev, savedGifticon]);
                  console.log('Successfully saved gifticon:', savedGifticon.id);
                }
              } else {
                console.log('Duplicate barcode found, skipping:', extractedInfo.barcodeValue);
                if (isMountedRef.current) {
              ...
              // 바코드가 없는 경우
              console.log('No barcode found in image, skipping');
              if (isMountedRef.current) {
                setSkippedOrDuplicateGifticons(prev => [...prev, extractedInfo]);
              }
            }
          } else {
            console.log('OCR failed, no data extracted');
          }
