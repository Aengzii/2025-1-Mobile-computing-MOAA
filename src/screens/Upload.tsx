...
const AVAILABLE_ICONS = [
    ...
    'gift-outline',         // 선물
    'popcorn',              // 팝콘
    'gamepad-variant-outline',// 게임
    'ticket-outline',       // 티켓
    'run',                  // 레저/스포츠
    'silverware-fork-knife' // 식사
];
const AVAILABLE_COLORS = [ '#FF69B4', '#FFD700', '#32CD32', '#4169E1', '#964B00', '#8A2BE2' ];


// --- 재사용 컴포넌트 ---
const InfoRow = ({ label, value, placeholder, isEditable = false, onChangeText, valueAlign, ...props }: any) => {
...
export const UploadScreen = () => {
  ...
  useEffect(() => {
    const fetchAndSetData = async () => {
      ...
      if (route.params) {
        const { imageUri, gifticonToEdit, extractedData } = route.params;
        if (gifticonToEdit) {
          ...
          // OCR/바코드 스캔 후 데이터 처리
          setImageUri(imageUri || extractedData.imageUri);
          if (extractedData.barcodeValue) setBarcode(extractedData.barcodeValue);
          if (extractedData.brandName) setStoreName(extractedData.brandName);
          ...
          if (extractedData.amount) setMemo(`금액: ${extractedData.amount}`);
          // 금액권 여부 자동 설정 (금액 정보가 있으면 금액권으로 설정)
          if (extractedData.amount) setIsVoucher(true);
        } else if (imageUri) { 
  ...
  const handleRegister = async () => {
    // 1. 기본 유효성 검사 (상품명, 유효기간)
    if (!productName || !expiryDate) {
      Alert.alert('입력 오류', '상품명과 유효기간은 필수 항목입니다.');
      return;
    }

    // 2. 실제 저장 로직을 별도 함수로 분리 (재사용을 위해)
    const proceedToSave = async () => {
      ...
      try {
        ...
        Alert.alert('오류', '저장/수정 중 문제가 발생했습니다.');
      }
    };

    // 3. 수정 모드가 아닐 경우에만 바코드 중복 검사 실행
    if (!gifticonToEdit) {
      const isDuplicate = await checkDuplicateBarcode(barcode);
      if (isDuplicate) {
        // 4. 중복 발견 시 사용자에게 확인 질문
        Alert.alert(
          '중복 확인',
          '이미 동일한 바코드의 기프티콘이 저장되어 있습니다. 계속하시겠습니까?',
          [
            {
              text: '아니오',
              style: 'cancel',
            ...
            {
              text: '예',
              onPress: proceedToSave, // '예'를 누르면 저장 로직 실행
            },
          ]
        );
      } else {
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoSection}>
          ...
          <TouchableOpacity onPress={() => setOpenDatePicker(true)}>
            <InfoRow label="유효기간" value={expiryDate} placeholder="YYYY-MM-DD" isEditable={false} />
          </TouchableOpacity>
        </View>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setCategoryModalVisible(false)}>
          <Pressable style={styles.modalContent} onPress={() => {}}>
            ...
            {searchQuery.length > 0 ? (
              <View style={styles.creationContainer}>
                ...
                <View style={styles.gridContainer}>
                  {AVAILABLE_ICONS.map(iconName => (
                    <TouchableOpacity 
                      key={iconName} 
                      style={[styles.gridItem, selectedIcon === iconName && styles.gridItemSelected]} 
                      onPress={() => setSelectedIcon(iconName)}
                    >
                      <Icon name={iconName} size={28} color={COLORS.gray8} />
                    </TouchableOpacity>
                  ))}
                </View>
              ...
              <FlatList data={allCategories} renderItem={renderCategoryItem} keyExtractor={item => item.id.toString()} style={styles.categorySelectList} />
            )}
          </Pressable>
        </Pressable>
      </Modal>
