  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.scanOptionsContainer}>
        ...
        />
        {/* 새로운 버튼: 전체 갤러리 전체 다시 스캔 */}
        <CustomButton
          title="전체 갤러리 전체 다시 스캔"
          onPress={() =>
            handleScan({
              scanMode: 'newInGallery', // 이 모드와 forceRescanProcessed: true 조합으로 전체 스캔
              forceRescanProcessed: true, // 모든 이미지 강제 재처리
            })
          }
          disabled={isScanning}
          buttonStyle={[styles.scanButton, { backgroundColor: COLORS.warning }]} // 주황색으로 구분
        />
        <CustomButton
